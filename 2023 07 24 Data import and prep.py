...
# Ensure data is structured so that each row represents a species and each column represents a timepoint

tomics = pd.read_csv("C:\\Users\\Alex Bennett\\Desktop\Python\\Omics integration\\Distance calculations\\data\\renal testing\\renal tomics small.csv")
...
pomics = pd.read_csv("C:\\Users\\Alex Bennett\\Desktop\Python\\Omics integration\\Distance calculations\\data\\renal testing\\renal pomics small.csv")
...
gomics = pd.read_csv("C:\\Users\\Alex Bennett\\Desktop\Python\\Omics integration\\Distance calculations\\data\\renal testing\\renal gomics small.csv")
...
momicszs = momicszsdf.to_numpy()
    ...
    #TODO:  Zscores from each dataset need to be aggregated before distance calculations perofrmed.


## Calculate distances for a single timepoint
#tomicsdist = []
...
## The code below produces a pair-wise distance matrix at each timepoint (t defined by column)
momicsdist = []
...
#Threshold the timeaveraged distance matrix:
threshold = AvDist['Distance'].quantile(0.2)
...
## A problem to consider:is smallest z-score-distance the best way to detemine co-regulation? Genes with similar waveforms but at different amplitudes/magnitudes of expression may have large distances but be coregulated.
## In response to above: Z-scores standardises for differences in expression magnitude, should be okay.

## Creating a network graph
## Unlabelled graph
#G = nx.Graph()
#for _, row in CoRegDists.iterrows():
#    gene1 = row['i']
#    gene2 = row['j']
